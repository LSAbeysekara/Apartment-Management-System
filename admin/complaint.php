...
$pendingRequestsQuery = "
    ...
    FROM tbl_complaint tm
    JOIN tbl_customer tc ON tm.cus_id = tc.cus_id
    WHERE tm.status = 'Pending' order by tm.id desc";
...
function getMaintenanceImages($com_id)
{
    // Use absolute path from server root
    $imageDir = realpath("../assets/images/complaint/$com_id");
    ...
    // Filter out the image files and construct proper URLs
    $images = [];
    foreach ($files as $file) {
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            ...
            $webPath = "../assets/images/complaint/$com_id/" . basename($file);
            $images[] = $webPath;
        }
    }
            ...
            <?php while ($request = mysqli_fetch_assoc($pendingRequests)): ?>
                <div class="card" style="padding: 2px;" id="colors">
                    <div style="border-radius:15px; padding: 5px;" >
                        ...
                        <div class="form-container">
                            <?php
                            $images = getMaintenanceImages($request['com_id']);
                            if (!empty($images)) {
                        ...
                        <div class="status-select">
                            <select name="status" data-id="<?= $request['com_id'] ?>" class="status-dropdown" id="status1">
