    ...
    <div class="form-container">
        <div class="table-wrapper">
            <div class="header-container">
                <a href="new-electricity.php" class="back-link"><span class="material-symbols-outlined">arrow_back</span></a>
            </div>

            <?php
            $sql = "SELECT COUNT(*) AS request_count FROM tbl_customer WHERE Electricity = 'request'";
            ...
            if ($result) {
                ...
                if($requestCount > 0) {
                ?>
                <h2>Electricity New Requests</h2>
        ...
        <div class="table-wrapper">
            <div class="header-container">
                <h2>Add New Electricity Account</h2>
            </div>
...
<script>
    ...
    $(document).on('click', '.btn-save', function () {
        ...
        $.ajax({
            url: 'approve-request-electricity.php',  // PHP file that will handle the approval
            method: 'POST',
            data: { cus_id: cusId, bill_type: 'Electricity' },
            dataType: 'json',
    ...
    $('#submit-all').on('click', function () {
        if (confirm('Are you sure you want to approve all requests?')) {
            $.ajax({
                url: 'approve-request-electricity-all.php',  // PHP file that will handle the approval for all
                method: 'POST',
                dataType: 'json',
                success: function (response) {
                    alert(response.message);
    ...
    $(document).on('click', '.btn-cancel', function () {
        ...
        if (confirm('Are you sure you want to deny this request?')) {
            $.ajax({
                url: 'deny-request-electricity.php',  // PHP file that will handle the denial
                method: 'POST',
                data: { cus_id: cusId, bill_type: 'Electricity' },
                dataType: 'json',
                success: function (response) {
    ...
    // Handle Add to Electricity button click
    $('#non-joined').on('click', '.btn-add', function () {
        const cusId = $(this).data('cus-id');
        const billType = 'Electricity'; // Define bill type here

        $.ajax({
            url: 'add_to_electricity.php',
            method: 'POST',
