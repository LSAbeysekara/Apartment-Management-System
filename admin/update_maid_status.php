...
$checkQuery = "SELECT m_id, status FROM tbl_maid WHERE m_id = ?";
...
if ($result->num_rows === 0) {
    $response['message'] = 'Maid pass not found';
    echo json_encode($response);
    $checkStmt->close();
    exit;
}
...
if ($currentRecord['status'] !== 'Pending') {
    $response['message'] = 'Can only update pending Maid passes';
    echo json_encode($response);
    $checkStmt->close();
    exit;
}
...
try {
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $response['message'] = "Maid pass {$v_id} has been {$status}";
            
            // Log the successful update
            error_log("Successfully updated Maid pass {$v_id} to {$status} by {$staff}");
        } else {
            $response['message'] = 'No changes made - Maid pass may already be processed';
            error_log("No rows updated for Maid pass {$v_id} - possible concurrent update");
        }
    } else {
        $response['message'] = 'Database update failed';
        error_log("Update failed for Maid pass {$v_id}: " . $stmt->error);
    }
